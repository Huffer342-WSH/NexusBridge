...
“设置 → 媒体库”的 `interval_minutes` 同样保存到 SQLite `app_settings`，由所有媒体库共用。默认 `360` 分钟（6 小时），范围 `0` 至 `1440`；`0` 关闭周期字幕扫描，但保留启动扫描、人工重扫和播放时按需生成。保存后立即生效，不写入主 JSON 配置。
...
`domain_suffix` 只填写主机名，不包含协议、端口、路径或通配符；匹配主机本身及其点分隔子域。`cookie_names` 是区分大小写的名称白名单，禁止留空。站点存在多个图片或下载资源域时可以配置多条规则，多个规则命中时采用最长后缀。站点 `domain` 同主机默认使用该站点全部 Cookie；封面位于未匹配的外部图床时不发送站点 Cookie。指定 Cookie 缺失时仍尝试无 Cookie 请求并在日志中记录缺失名称。重定向到新主机时会重新匹配规则，不继承上一跳 Cookie。
...
媒体页的列表/卡片布局、卡片宽度和标题展示策略保存在浏览器 `localStorage`，不新增服务端配置。封面首次显示时复用对应站点已有的 cookie、user-agent 和 Referer 下载，并持久化到 SQLite 同级的 `covers/`；新下载的原图会统一编码为 WebP，高度超过 1440 像素时先等比缩小。数据库只在 `cover_cache` 中保存相对路径和校验信息，不保存第二份凭据。封面下载使用 90 秒独立超时；超时或请求取消保留为可重试状态，其他错误在同一下载策略版本中不会自动重试。地址变化或应用升级了下载策略时会尝试更新，失败时继续使用仍然有效的旧缓存。封面 API 使用内容摘要作为 ETag，并返回 `public, max-age=86400`，因此部署在共享代理之后时图片可能进入共享缓存。
