...
媒体页的列表/卡片布局、卡片宽度和标题展示策略保存在浏览器 `localStorage`，不新增服务端配置。封面首次显示时复用对应站点已有的 cookie、user-agent 和 Referer 下载，并持久化到 SQLite 同级的 `covers/`；数据库只在 `cover_cache` 中保存相对路径和校验信息，不保存第二份凭据。相同封面地址在同一下载策略版本中失败后不会自动重试；地址变化或应用升级了下载策略时会尝试更新，失败时继续使用仍然有效的旧缓存。封面 API 使用内容摘要作为 ETag，并返回 `public, max-age=86400`，因此部署在共享代理之后时图片可能进入共享缓存。
