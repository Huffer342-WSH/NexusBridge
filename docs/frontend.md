...
播放页采用连续分栏区块。左侧是媒体画布和可选种子简介；右侧使用“剧集 / 选集 / 文件”标签窗格，其中“剧集”仅在剧集路由显示，“选集”仍只显示 qB 文件，“文件”浏览当前目录。文件标签中的 `..` 可进入父目录，列表使用资源管理器式紧凑行；从剧集或文件窗格切换媒体时保留当前浏览目录，只局部更新播放器及关联信息。右侧下方展示其他可播放种子。纯本机文件没有种子详情和 qB 选集。可用视频在主文件页、文件选择器和这三个右侧标签中使用固定尺寸缩略图占位，浏览器原生懒加载；加载或生成失败时保持电影图标。
