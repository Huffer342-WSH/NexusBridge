...
视频缩略图模块为本机可读视频生成单张 JPEG 列表预览。它复用现有文件和 qB 播放边界，只在浏览器真正请求图片时调用 FFprobe/FFmpeg；清单查询、剧集扫描和应用启动都不会预生成缩略图。

## 功能边界

- 展示范围包括主文件浏览页、文件选择器，以及播放器右侧“剧集 / 选集 / 文件”列表。
- 目录、音频、图片、普通文件和不可用视频继续显示原图标。
- 每个视频只生成一张静态 JPEG，不生成时间轴、雪碧图、动画预览或媒体库封面。
- 不运行后台预生成、定时刷新、文件系统 watcher 或全局缓存回收任务。
- 缩略图失败不影响目录浏览、剧集扫描或源文件播放。
...
flowchart TB
    ...
    Lists -->|可读视频附加 thumbnail_url| UI[VideoThumbnail 组件]
    UI -->|原生懒加载| HTTP[缩略图 HTTP API]
    ...
    HTTP -. 错误 .-> Icon[前端保留视频图标]
    Generate -. 错误 .-> Icon
```

清单模型中的 `thumbnail_url` 只是图片入口，不代表缓存已经存在。这样大量目录和剧集可以快速返回，FFmpeg 工作量由浏览器可见区域和原生懒加载控制。

## 清单字段与前端行为

以下模型增加可选 `thumbnail_url`：

| 模型 | 使用位置 |
| --- | --- |
| `FileEntry` | 主文件页、文件选择器、播放器“文件”标签 |
| `PlaybackMedia` | 播放器“选集”标签 |
| `PlaybackDirectoryFile` | 播放上下文中的当前目录快照 |
| `SeriesVideo` | 播放器“剧集”标签 |
...
2. 文件当前存在并且是普通文件。
3. 当前进程可以打开文件读取。
4. qB 来源的文件具有已下载数据，并通过保存目录边界校验。
5. 剧集来源的视频当前标记为可用。

`VideoThumbnail.vue` 使用固定尺寸占位，内部图片设置 `loading="lazy"` 和 `decoding="async"`。图片成功加载后覆盖电影图标；HTTP 错误、损坏响应或生成失败时只隐藏图片，行高、点击区域和播放操作保持不变。
...
已删除视频遗留的缓存不做后台 GC。管理员可以在应用停止后删除整个 `thumbnails/` 目录，后续图片请求会按需重建；该操作不会影响数据库、剧集配置或媒体源文件。
...
- 不写失败状态或负缓存。没有当前成功缓存时，每个新的图片请求都会重新尝试。
...
- 缩略图接口只能读取现有播放服务已经允许定位的本机文件，不增加远程路径映射。
- 本机文件 URL 和错误信息可能包含绝对路径，不应公开分享含敏感目录名的地址。
- 模块不修改、移动、删除源视频，也不调整 qB 文件优先级。
- 缓存目录只保存派生 JPEG，可以整体删除重建。
- 缩略图不是转码回退；浏览器仍直接解码原始媒体流。
- Docker 静态构建成功只能证明包和接口存在，不能证明所有编码都能正确取帧。

人工验收应覆盖 MP4/MKV、长短视频、首次生成、缓存命中、源文件变化、多视频并发、部分下载文件，以及 FFmpeg/FFprobe 缺失和损坏视频的图标回退。
...
| 前端固定占位和错误回退 | `webui/src/components/VideoThumbnail.vue` |
