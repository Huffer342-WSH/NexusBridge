...
- 视频展示范围包括主文件浏览页、文件选择器，以及播放器右侧“剧集 / 选集 / 文件”列表。
- 图片展示范围包括主文件浏览页、文件选择器和播放器右侧“选集 / 文件”列表。
...
| 模型 | 使用位置 |
| --- | --- |
| `FileEntry` | 主文件页、文件选择器、播放器“文件”标签；视频和图片均可存在 |
| `PlaybackMedia` | 播放器“选集”标签；视频和图片均可存在 |
...
qB 选集图片则直接复用对应 `PlaybackMedia.stream_url`，即数据库种子或 qB-only 的既有媒体流接口。这些接口返回原图片内容，并复用文件索引或本机路径校验、MIME、条件请求和 Range 语义。服务端不缩放、不转码、不写入 `thumbnails/`；大图片仍可能传输完整原文件，因此前端保留懒加载，只提高已触发请求的优先级。
