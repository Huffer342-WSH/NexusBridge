...
flowchart LR
    ...
    Context --> SeriesFiles[选集]
    Context --> Episodes[种子文件]
    Context --> Browser[浏览]
```
...
- `files`：qB 媒体文件；单文件上下文只有当前文件。
- `current_file_index`、`default_file_index`：当前和默认媒体。
- `current_path`、`current_directory`：当前本机文件及目录。
- `directory_files`：清单生成时的目录快照；前端“浏览”标签后续使用文件浏览 API 导航。
- `series`、`series_files`：仅剧集入口存在，分别提供剧集摘要和按目录顺序排列的视频缓存。

“种子文件”展示固定扩展名白名单识别出的图片、视频和音频。文件进度大于零且同机文件可读取时允许尝试播放；零进度禁用。默认文件依次选择完整视频、完整音频、完整图片，没有完整文件时选择进度最高的可用文件。
...
- 右侧上方是一个带“选集 / 种子文件 / 浏览”标签的媒体浏览区块。
- “选集”只在剧集入口显示并作为默认标签；暂时不可用的视频保留在清单中但不可点击。
- “种子文件”只在存在 qB 上下文时显示。
- “浏览”使用 `POST /api/files/browse` 读取当前目录；完整路径限制在窗格宽度内并以省略号截断，悬停可查看原值。
- 文件列表采用类似 Windows 文件资源管理器的紧凑行布局，没有卡片边框或明显的文件间分界；`..` 导航父目录，文件夹只负责继续浏览，不显示播放按钮。
- 从“浏览”窗格切换媒体时保留当前标签和浏览目录。旧播放器、简介和侧栏会持续显示到新上下文返回，只在播放器内显示局部切换状态，避免整页加载闪烁。
- 右侧下方是其他含完整音视频的数据库种子；纯本机文件不显示。
- 窄屏按媒体画布、简介、媒体浏览、其他种子的顺序纵向排列。

主文件页、文件选择器及右侧“选集 / 种子文件 / 浏览”为当前可读取的视频返回 `thumbnail_url`；文件浏览条目和 qB 媒体文件还会为图片返回直接指向对应受控原文件接口的 URL，不创建图片缩略图缓存。共享 `MediaThumbnail` 组件使用固定占位、原生 `loading=lazy` 和异步解码；图片请求使用较高优先级且不等待淡入动画。目录、音频、普通文件、不可用媒体及缩略图请求失败时继续显示原图标。清单 API 只拼接 URL，不运行 FFmpeg。
...
文件浏览器中的图片通过 `/api/playback/file/media?path=...` 懒加载原文件，qB 种子文件图片复用自身 `stream_url`；两者都复用既有路径和媒体白名单校验，不经过 FFprobe/FFmpeg，也不写入 `thumbnails/`。以下流程只适用于视频：
...
每个剧集可以独立启用保守的集数识别。识别只按同一扫描根目录和相对父目录分析缓存视频，图片等非视频不会参与覆盖比例；高相似文件名中近连续的数字被派生为集数，紧随其后的 `v2` 等标为修订版本。播放页选集卡片展示集数和去除组内重复片段后的短标题，完整文件名仍用于悬停提示。算法和阈值见[本地剧集](series.md#集数识别)。
