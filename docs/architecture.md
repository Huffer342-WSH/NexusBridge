...
| `internal/core/site_fetch.go`             | 站点分页扫描、持久化任务状态、增量边界和逐页订阅触发。 |
...
| `internal/storage/site_fetch_jobs.go`                | 站点扫描任务的查询、恢复和历史清理。                 |
...
| `webui/src/composables/useSiteFetchJobs.ts`        | 轮询站点扫描任务、有限退避并清理定时器。    |
