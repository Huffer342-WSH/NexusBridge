...
- 新增按需视频缩略图：文件页、文件选择器及播放器右侧“剧集 / 选集 / 文件”列表会懒加载本地 JPEG 缓存；首次请求使用 FFprobe/FFmpeg 取帧，失败时静默保留视频图标。Docker 镜像内置动态链接的 FFmpeg CLI 与 libav 运行库。
- 文件浏览器和播放器“选集”支持懒加载图片预览，直接通过现有受控媒体接口读取原图片，不生成或保存独立图片缩略图；图片请求使用较高优先级且不等待淡入动画。
