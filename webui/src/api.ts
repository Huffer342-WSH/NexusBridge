...
export const api = {
	...
	getQBCategories: (refresh = false) => request<QBCategoriesResult>(`/api/qb/categories${refresh ? '?refresh=true' : ''}`),
	...
	getSubscriptionCandidates: (subscriptionID: string, status?: string) =>
		request<SubscriptionCandidate[]>(`/api/subscriptions/${encodeURIComponent(subscriptionID)}/candidates${status ? `?status=${encodeURIComponent(status)}` : ''}`),
	getSubscriptionRuns: (subscriptionID?: string) =>
		request<SubscriptionRun[]>(`/api/subscription-runs${subscriptionID ? `?subscription_id=${encodeURIComponent(subscriptionID)}` : ''}`),
