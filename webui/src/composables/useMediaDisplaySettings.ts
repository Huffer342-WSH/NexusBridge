...
function normalizeSettings(value?: Partial<MediaDisplaySettings>): MediaDisplaySettings {
	...
	return {
		...
		cardMinWidth: Number.isFinite(cardMinWidth) && cardMinWidth >= cardWidthLimits.min && cardMinWidth <= cardWidthLimits.max
			? cardMinWidth
			: cardWidthLimits.default,
		titleMode: value?.titleMode === 'full' || value?.titleMode === 'scroll' ? value.titleMode : 'truncate',
		titleLines: Number.isFinite(titleLines) && titleLines >= titleLineLimits.min && titleLines <= titleLineLimits.max
			? titleLines
