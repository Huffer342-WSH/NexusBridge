...
import { CheckCircle2, Clock3, CloudDownload, Database, Eye, Plus, RefreshCw, Save, Tags, Trash2 } from '@lucide/vue';
...
const hasSiteFilters = computed(() =>
  Boolean(
    ruleDraft.value.site_categories.length ||
    ruleDraft.value.site_tags.length ||
    ruleDraft.value.subtitle_tags.length ||
    ruleDraft.value.promotions.length,
  ),
);
const selectedSiteForFacets = computed(() =>
  ruleDraft.value.site_ids.length === 1 ? ruleDraft.value.site_ids[0] : '',
);
const mergeSelectedOptions = (options: { label: string; value: string }[], selected: string[]) => {
  ...
  return [
    ...options,
    ...selected
      .filter((value) => !values.has(value.toLocaleLowerCase()))
      .map((value) => ({ label: `${value}（历史值）`, value })),
  ];
};
const siteCategoryOptions = computed(() =>
  mergeSelectedOptions(ruleFilterOptions.value.site_categories, ruleDraft.value.site_categories),
);
const siteTagOptions = computed(() =>
  mergeSelectedOptions(ruleFilterOptions.value.site_tags, ruleDraft.value.site_tags),
);
const subtitleTagOptions = computed(() =>
  mergeSelectedOptions(ruleFilterOptions.value.subtitle_tags, ruleDraft.value.subtitle_tags),
);
const promotionOptions = computed(() =>
  mergeSelectedOptions(ruleFilterOptions.value.promotions, ruleDraft.value.promotions),
);
const minSizeValue = computed({
  get: () => (ruleDraft.value.min_size ? ruleDraft.value.min_size / sizeFactors[sizeUnit.value] : null),
  set: (value: number | null) => {
    ruleDraft.value.min_size = value ? Math.round(value * sizeFactors[sizeUnit.value]) : 0;
  },
});
const maxSizeValue = computed({
  get: () => (ruleDraft.value.max_size ? ruleDraft.value.max_size / sizeFactors[sizeUnit.value] : null),
  set: (value: number | null) => {
    ruleDraft.value.max_size = value ? Math.round(value * sizeFactors[sizeUnit.value]) : 0;
  },
});
const publishedWithinValue = computed({
  get: () =>
    ruleDraft.value.published_within_minutes
      ? ruleDraft.value.published_within_minutes / publishedFactors[publishedUnit.value]
      : null,
  set: (value: number | null) => {
    ruleDraft.value.published_within_minutes = value ? Math.round(value * publishedFactors[publishedUnit.value]) : 0;
  },
});
...
function reasonText(item: RulePreviewItem | SubscriptionPreviewItem) {
  const reasons = [...(item.reasons ?? []), ...('plan' in item ? (item.plan?.reasons ?? []) : [])];
...
async function refreshWorkspace() {
  ...
  props.sites.forEach((site, index) => {
    ...
    nextSchedules[site.id] =
      result?.status === 'fulfilled' ? result.value : { site_id: site.id, enabled: false, interval_seconds: 900 };
  });
...
async function saveRule() {
  ...
  try {
    ...
    const saved = normalizeRule(
      await (originalName ? api.updateRule(originalName, ruleDraft.value) : api.saveRule(ruleDraft.value)),
    );
...
async function previewRule(automatic = false) {
  ...
  try {
    ...
    if (!automatic)
      setFeedback('success', `预览完成：命中 ${rulePreview.value.matched}/${rulePreview.value.evaluated}`);
  } catch (error) {
    if (generation !== previewGeneration) return;
...
async function refreshQBCategories() {
  ...
  try {
    ...
    setFeedback(
      qbCategories.value.stale ? 'warning' : 'success',
      qbCategories.value.stale ? '已显示分类缓存' : 'qB 分类已同步',
    );
  } catch (error) {
          ...
          <div class="rules-workspace-grid">
              ...
              <div v-if="rules.length" class="entity-list" data-testid="rule-list">
                ...
                >
                  <span
                    ...
                    ><small>{{ rule.site_ids.map(siteName).join('、') || '全部站点' }}</small></span
                  >
                </button>
              </div>
            ...
            <NCard title="规则编辑器" size="small" data-testid="rule-editor">
              <NForm label-placement="top" class="compact-form">
                <NFormItem label="规则名称（唯一标识）"
                  ><NInput v-model:value="ruleDraft.name" placeholder="例如：近期免费 ASMR"
                /></NFormItem>
                <NFormItem label="站点（多个值为 OR）">
                  <NSelect
                    :value="ruleDraft.site_ids"
                    multiple
                    clearable
                    :options="siteOptions"
                    @update:value="updateRuleSites"
                  />
                  <small v-if="hasSiteFilters" class="muted">已使用站点来源条件，因此只能保留一个站点。</small>
                </NFormItem>
                <div class="form-grid form-grid-2">
                  <NFormItem label="站点分类（多个值为 OR）"
                    ><NSelect
                      v-model:value="ruleDraft.site_categories"
                      multiple
                      clearable
                      filterable
                      :disabled="!selectedSiteForFacets"
                      :options="siteCategoryOptions"
                  /></NFormItem>
                  <NFormItem label="促销（多个值为 OR）"
                    ><NSelect
                      v-model:value="ruleDraft.promotions"
                      multiple
                      clearable
                      filterable
                      :disabled="!selectedSiteForFacets"
                  ...
                  /></NFormItem>
                  <NFormItem label="站点标签（必须全部具备）"
                    ><NSelect
                      v-model:value="ruleDraft.site_tags"
                      multiple
                      clearable
                      filterable
                      :disabled="!selectedSiteForFacets"
                      :options="siteTagOptions"
                  /></NFormItem>
                  <NFormItem label="标签（来自副标题，必须全部具备）"
                    ><NSelect
                      v-model:value="ruleDraft.subtitle_tags"
                      multiple
                      clearable
                      filterable
                      :disabled="!selectedSiteForFacets"
                      :options="subtitleTagOptions"
                  /></NFormItem>
                </div>
                <NFormItem label="主标题逻辑表达式">
                  <NInput
                    v-model:value="ruleDraft.title_expression"
                    type="textarea"
                    :autosize="{ minRows: 2, maxRows: 4 }"
                    placeholder="A&B&C|D|E&!F&(!H|!I)"
                  />
                  <small class="muted"
                    >优先级：! 高于 &，& 高于 |；支持括号。包含运算符的关键词用双引号，例如 "A&B"。</small
                  >
                </NFormItem>
                <div class="metric-pairs">
                  <NFormItem label="体积范围">
                    <div class="range-input-row">
                      <NInputNumber v-model:value="minSizeValue" :min="0" placeholder="最小" /><span>至</span
                      ><NInputNumber v-model:value="maxSizeValue" :min="0" placeholder="最大" /><NSelect
                        v-model:value="sizeUnit"
                        :options="sizeUnitOptions"
                      />
                    </div>
                  </NFormItem>
                  <NFormItem label="做种数量"
                    ><div class="range-input-row">
                      <NInputNumber v-model:value="ruleDraft.min_seeders" :min="0" placeholder="最少" /><span>至</span
                      ><NInputNumber v-model:value="ruleDraft.max_seeders" :min="0" placeholder="最多" /></div
                  ></NFormItem>
                  <NFormItem label="下载数量"
                    ><div class="range-input-row">
                      <NInputNumber v-model:value="ruleDraft.min_leechers" :min="0" placeholder="最少" /><span>至</span
                      ><NInputNumber v-model:value="ruleDraft.max_leechers" :min="0" placeholder="最多" /></div
                  ></NFormItem>
                  <NFormItem label="完成数量"
                    ><div class="range-input-row">
                      <NInputNumber v-model:value="ruleDraft.min_snatches" :min="0" placeholder="最少" /><span>至</span
                      ><NInputNumber v-model:value="ruleDraft.max_snatches" :min="0" placeholder="最多" /></div
                  ></NFormItem>
                  <NFormItem label="发布时间"
                    ><div class="range-input-row published-range">
                      <span>最近</span
                      ><NInputNumber v-model:value="publishedWithinValue" :min="0" placeholder="不限" /><NSelect
                        v-model:value="publishedUnit"
                        :options="publishedUnitOptions"
                      /></div
                  ></NFormItem>
                </div>
                <div class="form-grid form-grid-2">
                  <NFormItem label="排序字段"
                    ><NSelect v-model:value="ruleDraft.sort_by" :options="sortOptions"
                  /></NFormItem>
                  <NFormItem label="排序方向"
                    ><NSelect v-model:value="ruleDraft.sort_direction" :options="directionOptions"
                  /></NFormItem>
                </div>
            ...
            <NCard
              class="preview-card rule-preview-side"
              title="草稿预览"
              size="small"
              data-testid="rule-preview-results"
            >
              <template #header-extra>
                <NSpace>
                  ...
                  <NButton
                    ...
                    @click="previewRule(false)"
                    ><template #icon><NIcon :component="Eye" /></template>刷新</NButton
                  >
                </NSpace>
              </template>
              ...
              <template v-if="rulePreview">
                ...
                <div class="result-table rule-preview-results">
                  <NPopover
                    v-for="item in rulePreview.items"
                    :key="torrentKey(item.torrent.site_id, item.torrent.id)"
                    trigger="hover"
                    placement="left"
                    :show-arrow="false"
                  >
                    <template #trigger
                      ><div class="result-row" :class="{ matched: item.matched }">
                        <NCheckbox
                        ...
                        />
                        <div>
                          <strong>{{ item.torrent.title }}</strong
                          ><small>{{ siteName(item.torrent.site_id) }} · {{ item.torrent.category || '无分类' }}</small>
                        </div>
                        <div class="preview-status-tags">
                          <NTag :type="item.matched ? 'success' : 'default'">{{ item.matched ? '命中' : '排除' }}</NTag
                          ><NTag size="small" :type="qbStateType(item.qb_state)">{{ qbStateText(item.qb_state) }}</NTag>
                        </div>
                        <p>{{ reasonText(item) }}</p>
                      </div></template
                    >
                    <div class="rule-hover-details">
                      <strong>筛选详情</strong>
                      <span>主标题：{{ item.torrent.title }}</span
                      ><span>表达式：{{ ruleDraft.title_expression || '不限' }}</span>
                      <span
                        >站点分类：{{ item.torrent.category || '无' }} / 规则
                        {{ selectedLabels(siteCategoryOptions, ruleDraft.site_categories) }}</span
                      >
                      <span
                        >站点标签：{{ item.torrent.tag_ids?.join('、') || '无' }} / 规则
                        {{ selectedLabels(siteTagOptions, ruleDraft.site_tags) }}</span
                      >
                      <span
                        >副标题标签：{{ item.torrent.tags?.join('、') || '无' }} / 规则
                        {{ selectedLabels(subtitleTagOptions, ruleDraft.subtitle_tags) }}</span
                      >
                      <span
                        >促销：{{ item.torrent.promotion || '普通' }} / 规则
                        {{ selectedLabels(promotionOptions, ruleDraft.promotions) }}</span
                      >
                      <span
                        >体积：{{ formatBytes(item.torrent.size_bytes || 0) }}；做种
                        {{ item.torrent.seeders || 0 }}；下载 {{ item.torrent.leechers || 0 }}；完成
                        {{ item.torrent.snatches || 0 }}</span
                      >
                      <span
                        >发布时间：{{ formatTime(item.torrent.published_at) }}；{{ qbStateText(item.qb_state) }}</span
                      >
        ...
        <NTabPane name="subscriptions" tab="自动订阅">
          <div class="workspace-two-pane">
            <NCard title="订阅" size="small">
              <template #header-extra
                ><NButton size="small" @click="newSubscription"
                  ><template #icon><NIcon :component="Plus" /></template>新建</NButton
                ></template
              >
              <div v-if="subscriptions.length" class="entity-list" data-testid="subscription-list">
                ...
                >
                  <span
                    ...
                    ><small>优先级 {{ subscription.priority }}</small></span
                  >
                  <NTag size="small" :type="subscription.enabled ? 'success' : 'default'">{{
                    subscription.enabled ? '启用' : '关闭'
                  }}</NTag>
                </button>
              </div>
            ...
            <NCard title="订阅编辑器" size="small" data-testid="subscription-editor">
              <NForm label-placement="top" class="compact-form">
                <div class="form-grid form-grid-3">
                  ...
                  <NFormItem label="自动执行"
                    ><NSwitch v-model:value="subscriptionDraft.enabled"
                      ><template #checked>启用</template><template #unchecked>关闭</template></NSwitch
                    ></NFormItem
                  >
                  <NFormItem label="筛选规则"
                    ><NSelect v-model:value="subscriptionDraft.rule_name" :options="ruleOptions"
                  /></NFormItem>
                  <NFormItem label="优先级"><NInputNumber v-model:value="subscriptionDraft.priority" /></NFormItem>
                  <NFormItem label="站点"
                    ><NSelect v-model:value="subscriptionDraft.site_ids" multiple :options="siteOptions"
                  /></NFormItem>
                </div>
                <div class="form-grid form-grid-2">
                  <NFormItem label="qB 分类（完整名称）">
                    <NSelect
                      v-model:value="subscriptionDraft.download.qb_category"
                      clearable
                      filterable
                      tag
                  ...
                  <NFormItem label="保存路径模板"
                    ><NInput
                      v-model:value="subscriptionDraft.download.save_path_template"
                      placeholder="D:/PT/{{site_name}}"
                  /></NFormItem>
                  <NFormItem label="qB 标签"
                    ><NDynamicTags v-model:value="subscriptionDraft.download.qb_tags"
                  /></NFormItem>
                  <NFormItem label="任务名称模板"
                    ><NInput v-model:value="subscriptionDraft.download.filename_template" placeholder="{{title}}"
                  /></NFormItem>
                  <NFormItem label="最大并发（0 不限）"
                    ><NInputNumber v-model:value="subscriptionDraft.download.max_concurrent" :min="0"
                  /></NFormItem>
                  <NFormItem label="每日配额（0 不限）"
                    ><NInputNumber v-model:value="subscriptionDraft.download.daily_limit" :min="0"
                  /></NFormItem>
                  <NFormItem label="添加后暂停"
                    ><NSwitch v-model:value="subscriptionDraft.download.paused"
                  /></NFormItem>
                </div>
                <p class="template-help muted">
                  可用占位符：site_id、site_name、torrent_id、category、category_query、rule_name、subscription_name、title、detail_title、subtitle；使用双花括号包裹。
                </p>
                <NSpace justify="space-between">
                  <NPopconfirm
                    v-if="subscriptions.some((item) => item.id === subscriptionDraft.id)"
                    @positive-click="removeSubscription(subscriptionDraft)"
                  >
                    <template #trigger
                      ><NButton type="error" secondary
                        ><template #icon><NIcon :component="Trash2" /></template>删除</NButton
                      ></template
                    >
                    确定删除此订阅？
                  </NPopconfirm>
                  <NSpace>
                    <NButton
                      :loading="action === 'preview-subscription'"
                      data-testid="preview-subscription"
                      @click="previewSubscription"
                      ><template #icon><NIcon :component="Eye" /></template>预览</NButton
                    >
                    <NButton
                      type="primary"
                      :loading="action === 'save-subscription'"
                      data-testid="save-subscription"
                      @click="saveSubscription"
                      ><template #icon><NIcon :component="Save" /></template>保存订阅</NButton
                    >
                  </NSpace>
                </NSpace>
              </NForm>
          ...
          <NCard
            v-if="subscriptionPreview"
            title="最终下载计划预览"
            size="small"
            class="preview-card"
            data-testid="subscription-preview-results"
          >
            <template #header-extra
              ><NTag type="success"
                >可执行 {{ subscriptionPreview.eligible }}/{{ subscriptionPreview.evaluated }}</NTag
              ></template
            >
            <div class="plan-list">
              <article
                v-for="item in subscriptionPreview.items"
                :key="torrentKey(item.torrent.site_id, item.torrent.id)"
                class="plan-item"
              >
                <header>
                  <strong>{{ item.torrent.title }}</strong
                  ><NTag :type="item.eligible ? 'success' : 'warning'">{{ item.eligible ? '可执行' : '阻塞' }}</NTag>
                </header>
                <div class="plan-grid">
                  <span>分类：{{ item.plan.category || 'qB 默认' }}</span
                  ><span>路径：{{ item.plan.save_path || '分类/默认路径' }}</span>
                  <span>名称：{{ item.plan.rename || item.plan.original_name || '原始名称' }}</span
                  ><span>标签：{{ item.plan.tags.join(', ') || '无' }}</span>
                </div>
          ...
          <NCard title="未读候选" size="small">
            <div v-if="candidates.length" class="audit-list">
              <div v-for="candidate in candidates" :key="torrentKey(candidate.site_id, candidate.torrent_id)">
                <span
                  ><strong>{{ torrentTitle(candidate.site_id, candidate.torrent_id) }}</strong
                  ><small>{{ siteName(candidate.site_id) }} · 顺序 {{ candidate.source_order }}</small></span
                >
        ...
        <NTabPane name="automation" tab="周期与 qB 资源">
          <div class="automation-grid">
              ...
              <div class="schedule-list">
                <article v-for="site in sites" :key="site.id">
                  <header>
                    ...
                    <NSwitch v-if="schedules[site.id]" v-model:value="schedules[site.id].enabled" />
                  </header>
                  <NFormItem v-if="schedules[site.id]" label="间隔（秒，60–86400）">
                    <NInputNumber
                      v-model:value="schedules[site.id].interval_seconds"
                      :min="60"
                      :max="86400"
                      :step="60"
                    />
                  ...
                  <div v-if="schedules[site.id]" class="schedule-meta muted">
                    <span>上次：{{ formatTime(schedules[site.id].last_run_at) }}</span
                    ><span>下次：{{ formatTime(schedules[site.id].next_run_at) }}</span>
                  </div>
                  <NAlert v-if="schedules[site.id]?.last_error" type="warning" :bordered="false">{{
                    schedules[site.id].last_error
                  }}</NAlert>
                  <NButton size="small" :loading="action === `schedule:${site.id}`" @click="saveSchedule(site.id)"
                    ><template #icon><NIcon :component="Clock3" /></template>保存周期</NButton
                  >
                </article>
              </div>
            </NCard>
            ...
            <NCard title="qB 分类" size="small">
              <template #header-extra
                ><NButton size="small" :loading="action === 'qb-categories'" @click="refreshQBCategories"
                  ><template #icon><NIcon :component="RefreshCw" /></template>同步</NButton
                ></template
              >
              <NAlert
                v-if="qbCategories.stale || qbCategories.error"
                :type="qbCategories.error ? 'warning' : 'info'"
                :bordered="false"
                >{{ qbCategories.error || 'qB 离线，当前展示最近一次缓存' }}</NAlert
              >
              <div class="category-tree" data-testid="qb-category-tree">
                <div
                  v-for="category in categoryRows"
                  :key="category.name"
                  :style="{ paddingLeft: `${category.depth * 18 + 10}px` }"
                >
                  <NIcon :component="Database" /><span
                    ><strong>{{ category.path_segments.at(-1) || category.name }}</strong
                    ><small>{{ category.name }} · {{ category.save_path || '默认路径' }}</small></span
                  >
                </div>
              </div>
              <NForm label-placement="top" class="resource-create-form">
                ...
                <NButton
                  type="primary"
                  :loading="action === 'create-category'"
                  data-testid="create-qb-category"
                  @click="createCategory"
                  ><template #icon><NIcon :component="Plus" /></template>创建完整分类</NButton
                >
              </NForm>
            </NCard>
            ...
            <NCard title="qB 标签" size="small">
              <template #header-extra
                ><NButton size="small" :loading="action === 'qb-tags'" @click="refreshQBTags"
                  ><template #icon><NIcon :component="RefreshCw" /></template>同步</NButton
                ></template
              >
              <div class="tag-cloud">
                <NTag v-for="tag in qbTags.items" :key="tag" type="info">{{ tag }}</NTag
                ><span v-if="!qbTags.items.length" class="muted">尚无标签</span>
              </div>
              <NFormItem label="新标签"><NDynamicTags v-model:value="newTags" /></NFormItem>
              <NButton type="primary" :loading="action === 'create-tags'" @click="createTags"
                ><template #icon><NIcon :component="Tags" /></template>创建标签</NButton
              >
            </NCard>
          </div>
          ...
          <NCard title="最近运行" size="small">
            <template #header-extra
              ><NButton size="small" @click="loadRuns"
            ...
            <div v-if="runs.length" class="audit-list" data-testid="subscription-runs">
              <div v-for="run in runs" :key="run.id">
                <span
                  ><strong>{{ run.trigger }} · {{ run.subscription_id || run.site_id || '批量' }}</strong
                  ><small>{{ formatTime(run.started_at) }}</small></span
                >
                <NTag :type="statusType(run.status)">{{ run.status }}</NTag>
                <small
                  >发送 {{ run.sent }} · 已有 {{ run.exists }} · 跳过 {{ run.skipped }} · 失败 {{ run.failed }}</small
                >
              </div>
            </div>
        ...
        <NTabPane name="batch" tab="批量下载">
          <div class="batch-grid">
            <NCard title="选择数据库种子" size="small">
              <NSelect
                v-model:value="selectedTorrentKeys"
                multiple
                filterable
                clearable
                :options="torrentOptions"
                placeholder="也可先在规则预览中多选"
                data-testid="batch-torrents"
              />
              <p class="muted batch-hint">
                已选择 {{ selectedTorrentKeys.length }} 项。预览不会写入任务或修改 qB，仅执行只读校验。
              </p>
            </NCard>
            <NCard title="下载配置来源" size="small">
              <NRadioGroup v-model:value="batchMode" name="batch-mode">
                <NRadioButton value="subscription">快速套用订阅</NRadioButton
                ><NRadioButton value="custom">临时配置</NRadioButton>
              </NRadioGroup>
              <NForm v-if="batchMode === 'subscription'" label-placement="top"
                ><NFormItem label="订阅"
                  ><NSelect v-model:value="batchSubscriptionID" :options="subscriptionOptions" /></NFormItem
              ></NForm>
              <NForm v-else label-placement="top" class="compact-form">
                <div class="form-grid form-grid-2">
                  <NFormItem label="qB 分类"
                    ><NSelect
                      v-model:value="batchOptions.qb_category"
                      clearable
                      filterable
                      tag
                      :options="categoryOptions"
              ...
              <NSpace justify="end">
                <NButton :loading="action === 'preview-batch'" data-testid="preview-batch" @click="previewBatch"
                  ><template #icon><NIcon :component="Eye" /></template>预览计划</NButton
                >
                <NPopconfirm @positive-click="executeBatch">
                  <template #trigger
                    ><NButton type="primary" :loading="action === 'execute-batch'" data-testid="execute-batch"
                      ><template #icon><NIcon :component="CloudDownload" /></template>确认执行</NButton
                    ></template
                  >
          ...
          <NCard v-if="batchPreview" title="批量计划预览" size="small" data-testid="batch-preview-results">
            <div class="plan-list">
              <article
                v-for="item in batchPreview.items"
                :key="torrentKey(item.torrent.site_id, item.torrent.id)"
                class="plan-item"
              >
                <header>
                  <strong>{{ item.torrent.title }}</strong
                  ><NTag :type="item.eligible ? 'success' : 'warning'">{{ item.eligible ? '可发送' : '阻塞' }}</NTag>
                </header>
                <div class="plan-grid">
                  <span>分类：{{ item.plan.category || '默认' }}</span
                  ><span>路径：{{ item.plan.save_path || '默认' }}</span
                  ><span>名称：{{ item.plan.rename || item.plan.original_name || '原始名称' }}</span
                  ><span>标签：{{ item.plan.tags.join(', ') || '无' }}</span>
                </div>
                <p class="muted">{{ reasonText(item) }}</p>
          ...
          <NAlert v-if="batchResult" :type="batchResult.failed ? 'warning' : 'success'" data-testid="batch-result">
            ...
            已尝试 {{ batchResult.attempted }}，发送 {{ batchResult.sent }}，已存在 {{ batchResult.exists }}，跳过
            {{ batchResult.skipped }}，失败 {{ batchResult.failed }}。
          </NAlert>
        </NTabPane>
      </NTabs>
    </NSpin>
