...
const matchedScanItems = computed(
  () => scanResult.value?.items.filter((item) => item.status === 'matched' && item.preview.matches.length === 1) ?? [],
);
...
const recoveryNeedsIndex = computed(
  () => recoveryMode.value === 'database' || recoveryMode.value === 'database_then_site',
);
...
function handleNavigationMouseButton(event: MouseEvent) {
  ...
  if (event.type === 'mousedown') {
    ...
    const hasInternalTarget = event.button === 3 ? backHistory.value.length > 0 : forwardHistory.value.length > 0;
...
async function recoverMatchedScanItems() {
  ...
  try {
    ...
    emit(
      'message',
      `批量恢复完成：成功 ${result.recovered}，需处理 ${result.needs_attention}，失败 ${result.failed}，跳过 ${result.skipped}`,
    );
...
<template>
  <section class="file-manager-page">
    <div class="file-toolbar">
      ...
      <NButton
        class="file-forward-button"
        aria-label="前进"
        title="前进（鼠标前进侧键）"
        :disabled="!canGoForward"
        data-testid="file-forward"
        @click="goForward"
      >
      ...
      <NInput
        ...
        placeholder="输入本机完整目录路径"
        @keyup.enter="browse(pathDraft)"
      />
    ...
    <NAlert
      v-else-if="sizeIndex"
      :type="sizeIndexReady && !sizeIndex.failed ? 'success' : 'warning'"
      :bordered="false"
      class="size-index-alert"
    >
    ...
    <NModal v-model:show="recoveryOpen" preset="card" title="尝试恢复 qB 任务" class="recovery-modal">
      ...
      <NAlert v-if="recoveryResult && !recoveryResult.started" type="warning" :bordered="false" class="modal-alert">
        任务只添加了一次并保持暂停。强制校验尝试 {{ recoveryResult.recheck_attempts }} 次，当前状态
        {{ recoveryResult.qb_status.state }}；可选择仍然启动，或仅删除 qB 任务并保留文件。
      </NAlert>
      <NForm label-placement="top">
        <NFormItem label="候选来源">
          <NRadioGroup v-model:value="recoveryMode">
            <NRadioButton v-for="option in recoveryModeOptions" :key="option.value" :value="option.value">{{
              option.label
            }}</NRadioButton>
      ...
      <NSpace>
        <NButton
          type="primary"
          :loading="previewing"
          :disabled="(recoveryMode === 'url' && !torrentURL.trim()) || (recoveryNeedsIndex && !sizeIndexReady)"
          data-testid="preview-recovery"
        ...
        <NTag
          v-if="recoveryPreview"
          :type="
            recoveryPreview.matches.length === 1 ? 'success' : recoveryPreview.matches.length ? 'warning' : 'default'
          "
        >
      ...
      <div v-if="matchOptions.length" class="match-list">
        <label v-for="match in matchOptions" :key="matchKey(match)" class="match-option">
          ...
          <span
            ...
            ><small
              >{{ match.torrent.site_id }}/{{ match.torrent.id }} · {{ match.file_count }} 文件 ·
              {{ match.source }}</small
            ></span
          >
        </label>
      ...
      <template #footer>
        <NSpace justify="end">
          <NButton
            v-if="recoveryResult?.can_delete"
            type="error"
            secondary
            :loading="recoveryActioning"
            @click="controlFailedRecovery('delete')"
            >删除任务，保留文件</NButton
          >
          <NButton
            v-if="recoveryResult?.can_start"
            type="warning"
            :loading="recoveryActioning"
            @click="controlFailedRecovery('start')"
            >仍然开始任务</NButton
          >
    ...
    <NModal v-model:show="scanOpen" preset="card" title="扫描可能丢失的任务" class="scan-modal">
      <div class="scan-toolbar">
        ...
        <span class="scan-web-option">
          <NSwitch
            v-model:value="scanWebSearch"
            :disabled="scanning || batchRecovering"
            data-testid="scan-web-search"
          />
          启用网页搜索
        </span>
      ...
      <NSpin :show="scanning">
        <p v-if="scanResult" class="scan-summary">
          已评估 {{ scanResult.evaluated }} 项，唯一匹配 {{ scanResult.matched }} 项<span v-if="scanResult.truncated"
            >，已达到上限</span
          >。
        </p>
        <NSpace v-if="scanResult && !batchRecoveryResult" align="center" class="scan-batch-actions">
          <NButton
            ...
            :disabled="!matchedScanItems.length"
            data-testid="recover-scan-batch"
            @click="recoverMatchedScanItems"
          >
            <template #icon><NIcon :component="RotateCcw" /></template>
        ...
        <NAlert
          v-if="batchRecoveryResult"
          :type="batchRecoveryResult.failed || batchRecoveryResult.needs_attention ? 'warning' : 'success'"
          :bordered="false"
          class="modal-alert"
        >
          批量恢复完成：成功 {{ batchRecoveryResult.recovered }}，需处理 {{ batchRecoveryResult.needs_attention }}，失败
          {{ batchRecoveryResult.failed }}，跳过 {{ batchRecoveryResult.skipped }}。
        </NAlert>
        <div v-if="batchRecoveryResult" class="batch-result-list">
          <div
            v-for="item in batchRecoveryResult.items"
            :key="`${item.path}:${item.site_id}:${item.torrent_id}`"
            class="batch-result-item"
          >
            <span
              ><strong>{{ item.path }}</strong
              ><small>{{ item.error || `${item.site_id}/${item.torrent_id}` }}</small></span
            >
            <NTag
              :type="item.status === 'recovered' ? 'success' : item.status === 'needs_attention' ? 'warning' : 'error'"
              >{{ item.status }}</NTag
            >
            <NSpace v-if="item.result?.can_start || item.result?.can_delete" size="small">
              <NButton
                v-if="item.result.can_delete"
                size="tiny"
                type="error"
                secondary
                :loading="batchRecoveryActioning === `${item.result.qb_status.hash}:delete`"
                @click="controlBatchRecovery(item, 'delete')"
                >删除任务</NButton
              >
              <NButton
                v-if="item.result.can_start"
                size="tiny"
                type="warning"
                :loading="batchRecoveryActioning === `${item.result.qb_status.hash}:start`"
                @click="controlBatchRecovery(item, 'start')"
                >仍然开始</NButton
              >
            </NSpace>
          </div>
        ...
        <div class="scan-list">
          <button
            v-for="item in scanCandidates"
            :key="item.path"
            type="button"
            class="scan-item"
            @click="recoverScanItem(item)"
          >
            ...
            >
            <NTag :type="item.status === 'matched' ? 'success' : item.status === 'ambiguous' ? 'warning' : 'error'">{{
              item.status
            }}</NTag>
          </button>
        </div>
...
.file-toolbar {
  display: grid;
  grid-template-areas: 'back forward up path open refresh scan';
  grid-template-columns: auto auto auto minmax(220px, 1fr) auto auto auto;
  gap: 8px;
  align-items: center;
}
.file-back-button {
...
.size-index-alert :deep(.n-alert-body__content) {
  width: 100%;
}
.match-list,
.scan-list {
...
@media (max-width: 900px) {
  .file-toolbar {
    grid-template-areas: 'back forward up refresh refresh' 'path path path path open' 'scan scan scan scan scan';
