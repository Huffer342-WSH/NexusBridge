...
const completedSize = computed(() => {
	...
	if (status.value?.amount_left !== undefined && totalSize.value > 0) return Math.max(0, totalSize.value - status.value.amount_left);
...
const sizeLabel = computed(() => {
	if (added.value && !complete.value) return `${formatByteSize(completedSize.value)} / ${formatByteSize(totalSize.value)}`;
