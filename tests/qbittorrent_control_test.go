// qbittorrent_control_test.go 验证 qBittorrent 单种子暂停、恢复和校验原生接口。
package tests
...
// TestQBittorrentStartStopRecheckTorrents 验证 start/stop/recheck 请求路径和 hashes 参数。
func TestQBittorrentStartStopRecheckTorrents(t *testing.T) {
	...
	if err := client.RecheckTorrents(t.Context(), hashes); err != nil {
		t.Fatal(err)
	}
	want := []requestRecord{
		{Path: "/api/v2/torrents/stop", Hashes: "abc|def"},
		{Path: "/api/v2/torrents/start", Hashes: "abc|def"},
		{Path: "/api/v2/torrents/recheck", Hashes: "abc|def"},
	}
