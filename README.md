...
- 从媒体页或文件页播放 NexusBridge 所在机器上的源文件，支持浏览器 Range 请求、MKV 内嵌文本字幕和手动关联外挂字幕，不进行转码；ASS/SSA 优先使用 JASSUB/libass 渲染，并保留 WebVTT 回退。
- 可把本机目录组织为任意层级的媒体库，子库继承父库设置；剧集作为叶子媒体库保留视频扫描、最后选集恢复和保守的集数识别。
